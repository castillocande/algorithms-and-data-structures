...
bool test_iter_insert_after_at_start() {
  ...
  *aux = 3;
  ...
  aux = malloc(sizeof(int));
  *aux = 2;
  tests_result &= test_assert("Se inserto un elemento delante de head", // no sería después?
...
bool test_iter_insert_before_at_end() {
  ...
  *aux = 3;
  ...
  printf("CURR (debería ser 3): %d\n", *((int *)list_iter_peek_current(iter)));

  aux = malloc(sizeof(int));
  *aux = 2;
  tests_result &= test_assert("Se inserto un elemento antes del final",
                              list_iter_insert_before(iter, aux));

  list_iter_backward(iter);
  printf("CURR (debería ser 2): %d\n", *((int *)list_iter_peek_current(iter)));
  list_iter_forward(iter);
...
bool test_iter_insert_before_at_head() {
  ...
  *aux = 1;
  tests_result &= test_assert("Se inserto un elemento detras de head",
                              list_iter_insert_before(iter, aux)); // acá no funcionó. insertar antes del head
  list_iter_destroy(iter);
...
bool test_iter_insert_empty_lists() {
  ...
  *aux = 1;
  ...
  //
  // tests_result &= test_assert("EL PRIMER Y ÚNICO ELEMENTO ES HEADL", (*(int *)list_peek_head(list)) == 1); 
  // tests_result &= test_assert("EL PRIMER Y ÚNICO ELEMENTO ES TAIL", (*(int *)list_peek_tail(list)) == 1);
  //
  ...
  *aux = 2;
  ...
  tests_result &=
      test_assert("El iterador está en el primero", list_iter_at_first(iter));
  list_iter_destroy(iter);
  aux = list_pop_tail(list); // segfault
  tests_result &=
      test_assert("Se remueve el ultimo elemento y es un 2", *aux == 2);
  free(aux);
